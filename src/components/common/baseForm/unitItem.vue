<template>
  <div style="display: inline-block">
    <span>
      <el-input v-model.number="number" placeholder="" style="width: 50px;" :disabled="disabled" @blur="numChange"></el-input>
  ...
  export default {
    ...
    methods: {
      numChange(val) {
        ...
        const test = (/(^[0-9]\d*$)/.test(this.number)) ? 1 : 0
        console.log(test)
        if (!test) {
          this.$message.error('数量只能为数字')
          this.number = 0
        }
      }
    }
